...
// Check if user has passed multiple topics
export const checkMultiTopicPassed = async (topicIds: string[]): Promise<Record<string, boolean>> => {
  try {
    const response = await testAPI.checkMultiTopicPassed(topicIds);
